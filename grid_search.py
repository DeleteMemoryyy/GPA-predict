...
if __name__ == '__main__':
    ...
    # print('svr_mse: {}'.format(metrics.mean_squared_error(y_valid,svr_y_valid_predict)))

    #%% RidgeCV search
    regr_cv = lm.RidgeCV(alphas=[0.1,1,6,11,16],scoring='neg_mean_squared_error')
    ...
    print('regr_score: {}'.format(regr.score(x_valid,y_valid)))
    print('regr_mse: {}'.format(metrics.mean_squared_error(y_valid,regr_y_valid_predict)))
    ...
    lsr_y_valid_predict = lsr.predict(x_valid)
    print('lsr_score: {}'.format(lsr.score(x_valid,y_valid)))
    print('lsr_mse: {}'.format(metrics.mean_squared_error(y_valid,lsr_y_valid_predict)))

    #%% ElasticNetCV search
    enr_cv = lm.ElasticNetCV(l1_ratio=0.5)
    enr_cv.fit(x_all_train, y_all_train)
    print('Best ElasticNet alpha: {0}, l1_ratio: {1}'.format(enr_cv.alpha_, enr_cv.l1_ratio_))

    #%% Elastic Net regression
    enr = lm.ElasticNet(alpha=enr_cv.alpha_, l1_ratio=enr_cv.l1_ratio_)
    enr.fit(x_train, y_train)
    enr_y_valid_predict = enr.predict(x_valid)
    print('enr_score: {}'.format(enr.score(x_valid,y_valid)))
    print('enr_mse: {}'.format(metrics.mean_squared_error(y_valid,enr_y_valid_predict)))
